...
public class TournamentManagement {
	...
	private final static int BRACKET_SIZE = 8;
	private final static int[] winnerSpots = {0,2,3,16,17,18,19,20,21,22,26,30};
	private final static int[] loserSpots = {1,28,29,4,5,6,7,24,25,23,27,31};
	...
	public Tournament updateTournament (HttpServletRequest request) { // Update tournament after a result has been posted (DAO call)
		...
		if (!idStr.isEmpty() && tournamentDao.exists(Integer.parseInt(idStr)) && !score0Str.isEmpty() && !score1Str.isEmpty()) {
			...
			String winner = "";
			String loser = "";
			int winnerSpot = winnerSpots[matchNumber];
			int loserSpot = loserSpots[matchNumber];
